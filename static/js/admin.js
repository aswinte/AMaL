document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderAdminCalendar() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const pasaran = AmalUtils.getPasaranJawa(currentDate);

            // 2. Ambil Hijriah menggunakan modul Utils terpusat
            let infoH = AmalUtils.getHijriahFromJangkar(currentDate, adminCalJangkar, "NASIONAL_MABIMS");

            // Perbaikan kecil format bulan untuk UI Kalender Admin agar tanpa tulisan tahunnya
            if (infoH) {
                infoH.bulan = infoH.bulan.replace(/\s\d{4}\sH/g, '').trim();
            }
            ...
            let iconBulanHtml = infoH ? AmalUtils.getMoonPhaseIcon(infoH.tgl, "16px", "admin") : "";
