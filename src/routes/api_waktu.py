...
# Route untuk mendapatkan waktu sistem (perangkat keras) saat ini
@api_waktu_bp.route('/api/get_hw_time', methods=['GET'])
def get_current_time():
    try:
        # Mengambil waktu lokal dari sistem
        now = datetime.now()
        return jsonify({
            "status": "success",
...
# Route untuk mengatur waktu sistem (perangkat keras)
@api_waktu_bp.route('/api/set_hw_time', methods=['POST'])
def set_system_time():
    try:
        data = request.json
        new_time = data.get("datetime") # Format: "YYYY-MM-DD HH:MM:SS"
        
        if not new_time:
            return jsonify({"status": "error", "message": "Format waktu tidak valid"}), 400

        # 1. Set waktu sistem menggunakan timedatectl
        # timedatectl set-time juga otomatis menyinkronkan ke RTC jika ada
        cmd = ["/usr/bin/sudo", "/usr/bin/timedatectl", "set-time", new_time]
        subprocess.run(cmd, check=True)

        return jsonify({
            "status": "success", 
            "message": f"Waktu sistem berhasil diperbarui ke {new_time}"
        })
    except subprocess.CalledProcessError as e:
        return jsonify({
            "status": "error", 
            "message": "Gagal mengatur waktu. Pastikan NTP dalam keadaan inactive."
        }), 500
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500